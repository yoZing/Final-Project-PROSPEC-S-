...
<!--                     <h1>자료게시판 수정</h1> -->
                ...
                <div class="col-sm-6 d-none d-sm-block">
					...
					<li class="breadcrumb-item active">수 정</li>
                    </ol>
                </div>
            </div>
        </div>
      ...
      <div class="card card-outline card-info">
			...
			<div class="card-body pad">
						<form enctype="multipart/form-data" role="form" method="post" action="modify.do" name="modifyForm">
							<input type="hidden" name="attachNo" value="${pds.attachNo }"/>
							<input type="hidden" name="pdsNo" value="${pds.pdsNo }" />
							...
							<div class="form-group">
								...
								<input type="hidden" id="register"
									name="register" class="form-control" value="${pds.register }" readonly>
								<input type="text" id="userNm"
									name="userNm" class="form-control" value="${pds.userNm }" readonly>
							</div>
							<div class="form-group">								
								<div class="card card-outline card-info">
									<div class="card-header">
										...
										<ul class="mailbox-attachments d-flex align-items-stretch clearfix">
											...
											<li class="attach-item thumb${attach.attachSeq }" file-name="${attach.nm }" target-attachSeq="${attach.attachSeq }">																			
												<div class="mailbox-attachment-info ">
													<a class="mailbox-attachment-name" name="attachedFile" attach-fileName="${attach.nm }" attach-attachSeq="${attach.attachSeq }" href="<%=request.getContextPath()%>/pds/getFile.do?attachSeq=${attach.attachSeq }"  >													
...
<script>
	window.addEventListener('DOMContentLoaded', modifyPds);
