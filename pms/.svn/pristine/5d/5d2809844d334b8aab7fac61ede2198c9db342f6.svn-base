...
public class MimeAttachNotifier {
	...
	public void sendMail(MailMessageCommand command) throws Exception {
		// 메시지 생성
		...
		// 보내는사람
		messageHelper.setFrom(command.getSender(), command.getSenderNm());
		// 제목
	...
	public void sendMail(MailMessageCommand command, String uploadPath) throws Exception {
		...
		if (command.getFile() != null && !command.getFile().isEmpty()) {
			...
			DataSource dataSource 
					= new FileDataSource(new File(uploadPath, fileName));
